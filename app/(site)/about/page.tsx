 ...
 return <>
  ...
  <section className="wrap section split" aria-labelledby="record-title">
   ...
   <div className="rule-list">
    {education.map(e=><div key={e.title} className="record-row"><span className="mono muted">{e.years}</span><span className="stack" style={{gap:6}}><span className="h-item">{e.title}</span><span className="text-2" style={{fontSize:15}}>{e.institution}</span></span>{e.result?<span className="chip-result">{e.result.toUpperCase()}</span>:<span/>}</div>)}
   </div>
  </section>
