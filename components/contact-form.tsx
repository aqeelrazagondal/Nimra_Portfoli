...
useEffect(()=>{if(state.status==='idle')return;if(widgetId.current)window.turnstile?.reset(widgetId.current);if(state.status==='success'){form.current?.reset();setType(enquiryTypes[0])}
...
<div className="form-grid"><label>Your name<input autoComplete="name" required maxLength={120} {...field('name')}/>{error('name')}</label><label>Email address<input autoComplete="email" type="email" spellCheck={false} required maxLength={200} {...field('email')}/>{error('email')}</label></div>
