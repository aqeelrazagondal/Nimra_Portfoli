...
export type ArticleSummary={
 ...
 cover:{src:string;alt:string;width:number;height:number;blurDataURL:string};tags:string[];series:string;seriesPart:number|null;
...
async function analyse(root:Node){
 ...
 for(const node of root.walk()){
  ...
  if(node.type==='tag'&&node.tag==='figure'){const size=await articleImage(String(node.attributes.src??''));node.attributes.w=size.width;node.attributes.h=size.height;node.attributes.blur=size.blurDataURL}
  if(node.type==='tag'&&node.tag==='pullQuote')words+=String(node.attributes.quote??'').split(/\s+/).filter(Boolean).length;
 }
...
export const markdocConfig:Config={
 ...
 tags:{
  ...
  figure:{render:'Figure',selfClosing:true,attributes:{src:str,alt:str,caption:str,credit:str,width:str,w:{type:Number},h:{type:Number},blur:str}},
...
async function load(slug:string,entry:Entry):Promise<Article>{
 ...
 const {words,footnotes,headings}=await analyse(node);
 const size=await articleImage(entry.coverImage);
 return {
  slug,title:entry.title,subtitle:entry.summary,excerpt:entry.summary,
  publishedAt:entry.publishedAt,updatedAt:entry.updatedAt||null,
  cover:{src:entry.coverImage,alt:entry.coverAlt,...size},tags:[...entry.tags],series:'',seriesPart:null,
  featured:entry.featured,wordCount:words,readingTime:Math.max(1,Math.ceil(words/225)),
  draft:entry.status!=='published',scheduled:false,
  coverCaption:entry.coverCaption,coverCredit:'',correction:'',references:[],
  citeable:true,canonicalUrl:null,seoTitle:'',seoDescription:'',
  content:Markdoc.transform(node,markdocConfig),footnotes,headings,
 };
}

const visible=(a:ArticleSummary)=>!a.draft;

// All articles readers can see here, newest first.
export const getArticles=cache(async():Promise<Article[]>=>{
 const entries=await reader.collections.articles.all();
 const articles=await Promise.all(entries.filter(e=>e.entry.status==='published').map(e=>load(e.slug,e.entry)));
