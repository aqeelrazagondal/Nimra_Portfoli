...
export async function ogImage({eyebrow,title,subtitle}:{eyebrow:string;title:string;subtitle:string}){
 const [fraunces,frauncesItalic,inter]=await Promise.all([font('Fraunces-Regular.ttf'),font('Fraunces-Italic.ttf'),font('Inter-Medium.ttf')]);
 return new ImageResponse(<div style={{width:'100%',height:'100%',display:'flex',flexDirection:'column',justifyContent:'space-between',padding:'72px 80px',background:'radial-gradient(ellipse at 85% 30%, #2c1f4d 0%, #0e0a18 60%)',color:'#ece8f4',fontFamily:'Inter'}}>
  ...
  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',fontSize:24,color:'#bdb6cc',borderTop:'1px solid #30263f',paddingTop:28}}><div style={{display:'flex',fontFamily:'Fraunces',fontSize:34,color:'#ece8f4'}}>Nimra Zahid<svg width="22" height="22" viewBox="0 0 24 24" style={{marginLeft:12,marginTop:4}}><path d="M12 2v20M2 12h20M4.9 4.9l14.2 14.2M19.1 4.9 4.9 19.1" stroke="#b9a2ff" strokeWidth="2.4" strokeLinecap="round"/></svg></div><div style={{display:'flex'}}>International Relations researcher & educator</div></div>
 </div>,{...ogSize,fonts:[{name:'Fraunces',data:fraunces,style:'normal',weight:400},{name:'Fraunces',data:frauncesItalic,style:'italic',weight:400},{name:'Inter',data:inter,style:'normal',weight:500}]});
}
